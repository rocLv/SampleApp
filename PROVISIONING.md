# How to provision a new environment for sample_app
...
* Enter domains to secure (for example): `sampleapp.com`
* Enter webroot path: `/home/deployer/apps/sample_app/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/sampleapp.com/fullchain.pem /etc/ssl/sample_app.crt
ln -s /etc/letsencrypt/live/sampleapp.com/privkey.pem /etc/ssl/sample_app.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/sample_app/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
21 5,17 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/sample_app/shared/public/
```
